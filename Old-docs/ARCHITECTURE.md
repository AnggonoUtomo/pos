...
CRUD master/operasional yang mutable memakai soft delete secara default.

Aturan:

- Tambahkan kolom `deleted_at` pada tabel master yang record-nya masih perlu histori referensi.
- Eloquent model memakai `SoftDeletes` jika model tersebut dapat dihapus dari UI.
- Delete action harus audited dan tidak boleh menghapus relasi historis penting.
- Restore dibuat ketika workflow bisnis membutuhkannya; jika belum ada UI restore, tetap catat keputusan pada work-item.
- Hard delete hanya untuk data disposable/test atau cleanup yang disetujui eksplisit.
- Transaksi posted, ledger stok/finance, stock movement, FIFO layer, payment record, transaction snapshot, dan activity log tidak memakai soft delete sebagai mekanisme koreksi histori. Gunakan void, reversal, retur, atau adjustment.
...
`can(permission)`, `canAny(permissionList)`, `hasRole(role)`, `isSuperSystem`,
`roles`, `permissions`, dan `user`.

Aturan:

- `can()` dan `canAny()` boleh menganggap `auth.superSystem === true` sebagai bypass UI.
...
Operasi CRUD pada UI admin/POS memakai Sonner toast dari shadcn/ui.

Aturan:

- Create/update/delete/restore sukses menampilkan toast sukses dengan label objek yang jelas.
- Error non-field memakai toast error; error validasi field tetap dekat input.
- Aksi destructive tetap memakai dialog konfirmasi sebelum request.
- Toast dipanggil setelah response server sukses atau gagal, bukan sebelum persistence selesai.
- Toast tidak boleh menjadi satu-satunya bukti aksi penting; backend tetap mencatat audit/activity log sesuai scope.
